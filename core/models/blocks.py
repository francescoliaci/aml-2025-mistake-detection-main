...
class LSTMBaseline(nn.Module):
    """
    Baseline LSTM per SupervisedER.
    Prende in input feature di dimensione (B, T, D) oppure (B, D)
    e produce un logit per ogni esempio (nessuna sigmoid: usiamo BCEWithLogitsLoss).
    """

    def __init__(self, input_size, hidden_size=256, num_layers=1, bidirectional=False):
        ...
        self.fc = nn.Linear(hidden_size * num_dirs, 1)

    def forward(self, x):
        """
        x: (B, T, D) o (B, D)
        """
        # Se è 2D, interpretiamo come sequenza di lunghezza 1
        if x.dim() == 2:
            x = x.unsqueeze(1)  # (B, 1, D)

        # LSTM
        _, (h_n, _) = self.lstm(x)
        # h_n: (num_layers * num_dirs, B, hidden_size)
        last_h = h_n[-1]  # (B, hidden_size * num_dirs)

        logits = self.fc(last_h)  # (B, 1)
